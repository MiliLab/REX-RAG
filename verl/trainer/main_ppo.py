...
class RewardManager():
    ...
    def __call__(self, data: DataProto, format_detail = False):
        ...
        if format_detail:
            res['reward_extra_info'] = format_detail_dict
        return res
...
def run_ppo(config) -> None:
    ...
    if not ray.is_initialized():
        ...
        ray.init(
            runtime_env={"env_vars": {"TOKENIZERS_PARALLELISM": "true", "NCCL_DEBUG": "WARN", "VLLM_LOGGING_LEVEL": "WARN", "VLLM_ALLOW_RUNTIME_LORA_UPDATING": "true","CUDA_DEVICE_MAX_CONNECTIONS": "1"}},
...
class TaskRunner:
    def run(self, config):
        ...
        val_reward_fn = RewardManager(tokenizer=tokenizer, num_examine=1, structure_format_score=config.data.structure_format_score)
